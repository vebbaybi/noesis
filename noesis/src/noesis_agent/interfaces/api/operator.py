...
def operator_status(request: Request) -> dict:
    ...
    return {
        ...
        "memory": get_container().memory.health(),
        ...
        "configuration_issues": [
            {"key": "discord" if issue.key == "DISCORD_BOT_TOKEN" else issue.key,
             "severity": issue.severity, "detail": issue.message}
            for issue in issues
        ],
...
def operator_memory_hygiene(request: Request) -> dict:
    _require_local(request)
    result = get_container().memory.run_hygiene()
    return {"completed": True, "result": result,
            "memory": get_container().memory.diagnostics()}
...
async def inspect_cognition(request: Request, event: MentionEvent) -> dict:
    ...
    return {
        ...
        "recent_context": {
            "used": bool(response.metadata.get("response_plan", {}).get("used_recent_context")),
            "raw_content_exposed": False,
        },
        "retrieval": response.metadata.get("retrieval", {}),
        "response_plan": response.metadata.get("response_plan", {}),
...
def operator_memory(request: Request, limit: int = 6) -> dict:
    ...
    return container.memory.operator_summary(preview_limit=limit)
...
def operator_ambient(request: Request) -> dict:
    ...
    memory_health = container.memory.health()["worker"]
    return {
        "observation_mode": settings.memory_observation_mode,
        "ambient_response_enabled": settings.ambient_response_enabled,
        "moderation_analysis_enabled": settings.moderation_analysis_enabled,
...
<nav aria-label="Operator sections"><a href="#overview">Overview</a> Â· <a href="#cognition">Cognition</a> Â· <span class="muted">Models Â· Memory Â· Sources Â· Audit (read-only status; controls planned)</span></nav>
<div class="grid"><section id="overview"><h2>Runtime readiness</h2><pre id="status">Loading safe statusâ€¦</pre><button onclick="loadStatus()">Refresh</button></section>
