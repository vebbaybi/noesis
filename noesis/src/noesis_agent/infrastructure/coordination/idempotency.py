...
class LocalIdempotencyStore:
    ...
    async def claim(self, tenant_id: str, event_id: str, *, ttl_seconds: int) -> bool:
        ...
        async with self._lock:
            self._keys = {
                item: expiry for item, expiry in self._keys.items() if expiry > now
            }
...
class RedisIdempotencyStore:
    def __init__(
        self, url: str, *, namespace: str = "noesis", operation_timeout: float = 1.0
    ) -> None:
        self._url = url
    ...
    async def claim(self, tenant_id: str, event_id: str, *, ttl_seconds: int) -> bool:
        if not self._url or importlib.util.find_spec("redis") is None:
            ...
            return await self._fallback.claim(
                tenant_id, event_id, ttl_seconds=ttl_seconds
            )
        try:
    ...
    async def validate_service(self) -> dict[str, object]:
        ...
        try:
            ...
            value, ttl = await asyncio.wait_for(
            ...
            self._last_success = (
                datetime.now(timezone.utc) if self._validated else self._last_success
            )
            return {
                "validated": self._validated,
                "state": "ready" if self._validated else "failed",
                "ttl_seconds": int(ttl),
    ...
    async def _round_trip(self, key: str) -> tuple[object, int]:
        if self._client is None:
            raise ConnectionError("Redis client is not initialized")
        await self._client.set(key, "ok", ex=5)
    ...
    def health(self) -> CapabilityHealth:
        state = (
            ...
            if self._degraded or not self._url
