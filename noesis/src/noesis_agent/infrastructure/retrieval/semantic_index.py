...
class _EmbeddingModel(Protocol):
    def query_embed(self, documents: Sequence[str]) -> Iterable[Sequence[float]]: ...
    def passage_embed(self, documents: Sequence[str]) -> Iterable[Sequence[float]]: ...
...
class SemanticIndex:
    ...
    def __init__(
        self, config: IndexConfig, *, enabled: bool = False, concurrency: int = 2
    ) -> None:
        self._config = config
        self._enabled = enabled
        self._slots = asyncio.Semaphore(concurrency)
        self._client: AsyncQdrantClient | None = None
        self._embedder: _EmbeddingModel | None = None
        self._ready = False
        self._failure = ""
        ...
        if self._config.collection not in names:
            await self._client.create_collection(
                ...
                vectors_config=models.VectorParams(
                    size=self._config.dimension, distance=models.Distance.COSINE
                ),
            )
            ...
            if size != self._config.dimension:
                raise ValueError(
                    f"Qdrant collection dimension {size} != configured {self._config.dimension}"
                )
        self._ready = True
    ...
    async def _embedding(self, text: str, *, query: bool) -> list[float]:
        async with self._slots:
            return await asyncio.get_running_loop().run_in_executor(
                None, self._embedding_sync, text, query
            )
    ...
    def _embedding_sync(self, text: str, query: bool) -> list[float]:
        if self._embedder is None:
            ...
            if self._config.model not in models:
                raise ValueError(
                    f"FastEmbed model is not supported: {self._config.model}"
                )
            if int(models[self._config.model]["dim"]) != self._config.dimension:
                raise ValueError(
                    "Configured embedding dimension does not match FastEmbed metadata"
                )
            self._embedder = cast(
                _EmbeddingModel, TextEmbedding(model_name=self._config.model)
            )
        method = self._embedder.query_embed if query else self._embedder.passage_embed
    ...
    async def retrieve(
        ...
        tenant_filter = models.Filter(
            must=[
                models.FieldCondition(
                    key="tenant_id", match=models.MatchValue(value=request.tenant_id)
                ),
                models.FieldCondition(
                    key="visibility",
                    match=models.MatchAny(any=["tenant", "conversation"]),
                ),
            ]
        )
        ...
        return [
            RetrievalItem(
                ...
                text=str(point.payload["text"])[:4000],
                score=max(0.0, min(1.0, float(point.score))),
                source_id=str(point.payload["source_id"]),
                provenance=str(point.payload["source_type"]),
            )
            for point in response.points
            if point.payload and point.payload.get("tenant_id") == request.tenant_id
        ]
    ...
    async def remember(self, request: IntelligenceRequest, text: str) -> None:
        ...
        point_id = str(
            uuid5(
                NAMESPACE_URL,
                f"{request.tenant_id}:{request.event_id}:response:{digest}",
            )
        )
        payload = {
    ...
    async def upsert_source(self, record: SemanticSourceRecord) -> str:
        ...
        if len(vector) != self._config.dimension:
            raise ValueError(
                f"Embedding dimension {len(vector)} != configured {self._config.dimension}"
            )
        await self._client.upsert(
            collection_name=self._config.collection,
            points=[models.PointStruct(id=point_id, vector=vector, payload=payload)],
                ...
                records.append(
                    VectorPointMetadata(
                        ...
                        embedding_dimension=int(
                            payload.get("embedding_dimension", self._config.dimension)
                        ),
    ...
    def health(self) -> CapabilityHealth:
        ...
        if self._failure:
            return CapabilityHealth(
                name="rag", state=CapabilityState.UNAVAILABLE, detail=self._failure
            )
        return CapabilityHealth(
            name="rag",
            state=CapabilityState.READY if self._ready else CapabilityState.LOADING,
        )
