...
from noesis_agent.models.runtime import HostIntent, ResponseMode
from noesis_agent.models.schemas import HostReplyRequest, HostReplyResponse
...
class HostService:
    def __init__(
        ...
        *,
        ...
        self.default_model = default_model or settings.runtime_profile.default_model or settings.default_model or "gpt-4.1-mini"
    ...
    async def generate_reply(
        ...
        system_parts = [
            ...
            *brain_context.system_notes,
        ]
        voice_prompt = self.brain.voice_profiles.build_style_prompt(payload.personality if payload.personality != "default" else None)
        if voice_prompt:
        ...
        if brain_context.recall_hits:
            user_parts.append("Relevant local memory:\n- " + "\n- ".join(brain_context.recall_hits[:4]))
        if brain_context.finance.glossary_hits:
            user_parts.append("Finance grounding:\n- " + "\n- ".join(brain_context.finance.glossary_hits))
        if brain_context.humor.hooks:
            user_parts.append("Humor guidance:\n- " + "\n- ".join(brain_context.humor.hooks[:2]))
            ...
            if self.cognition_provider is not None:
                cognition_response = await self.cognition_provider.generate(
                    CognitionRequest(
                        ...
                        context="\n\n".join(user_parts),
                        transcript_tail=transcript_tail,
                        memories=brain_context.recall_hits,
                        local_fallback=f"[OpenAI unavailable] {brain_context.fallback_reply}",
                        metadata={
                            "system_prompt": "\n".join(system_parts),
                ...
                reply_text = f"[OpenAI unavailable] {brain_context.fallback_reply}"
            cleaned = self._clean_reply(reply_text)
        except Exception as exc:
            self.logger.warning("Host LLM generation failed, using local brain reply", extra={"error": str(exc)})
            cleaned = f"[OpenAI unavailable] {brain_context.fallback_reply}"

        response = HostReplyResponse(
            text=cleaned,
            speakable_text=cleaned,
            tone_used=brain_context.policy.style,
            personality_used=brain_context.humor.style,
            suggested_length_category=payload.length_preset,
            contains_call_to_action="?" in cleaned or "check" in cleaned.lower(),
            should_pin=brain_context.finance.active and brain_context.finance.risk_level == "high",
        )
