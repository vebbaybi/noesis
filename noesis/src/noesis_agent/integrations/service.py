...
from noesis_agent.cognition.prompts.service import PromptConfig, build_announcement_prompt, build_nft_hashtag_prompt
from noesis_agent.integrations.llm.openai import OpenAIService
from noesis_agent.shared.noesislogger import NoesisLogger, get_noesis_logger
...
class PlatformService:
    def __init__(
        ...
        self.logger.info(f"PlatformService initialized â€” primary = {self.primary}")
    ...
    async def format_announcement(
        ...
        # Final truncation
        if len(text) > limit - 3:
            text = text[: limit - 5].rsplit(" ", 1)[0] + "â€¦"
    ...
    async def generate_hashtags(self, context: str, max_count: int = 6) -> List[str]:
        ...
        try:
            ...
            self.logger.warning("Hashtag generation failed â†’ fallback", extra={"error": str(e)})
    ...
    def prepare_x_thread_segment(
        ...
        if add_disclaimer_on_last and part == total:
            segment += "\n\nDYOR â€¢ NFA â€¢ Crypto/NFT markets are volatile"
        return segment
...
# Global singleton for legacy compatibility â€” prefer dependency injection
@lru_cache(maxsize=1)
